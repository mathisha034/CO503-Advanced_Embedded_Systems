...
int producer()
{
	...
	while(j<=500)
	{
		...
		total_time_write_next_input = ((unsigned long)(second_write - intial_write))/((unsigned long)timer_freq);
		single_total_time = ((unsigned long)(single_write_final - single_write_intial))/((unsigned long)timer_freq);

		printf("Producer sent [%i] : SWT = %lu, PT = %lu\n",j , (unsigned long)single_total_time, (unsigned long)total_time_write_next_input);
		j+=10;
	}
